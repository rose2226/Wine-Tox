...
model = xgb.Booster()
model.load_model("model/wine_quality_xgb.json")
    ...
    with st.form("input_form"):
        ...
        for i, feat in enumerate(feature_names):
            ...
            with col:
                ...
                default = default_values[feat]
                value = st.session_state.get(feat, default)
                st.session_state[feat] = col.slider(f"{feat}", min_value=float(min_val), max_value=float(max_val), value=float(value), step=float(step), key=feat)
